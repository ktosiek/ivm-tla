...
dmUserDoc ==
    BagMinus(
        BagPlus(UserDoc(dUserGroups, docGroups),
                UserDoc(userGroups, dDocGroups)),
        UserDoc(dUserGroups, dDocGroups))
...
Init ==
    /\ userGroups = [t \in (Users \X Groups) |-> 0]
    /\ dUserGroups = [t \in (Users \X Groups) |-> 0]
    /\ docGroups = [t \in (Documents \X Groups) |-> 0]
    /\ dDocGroups = [t \in (Documents \X Groups) |-> 0]
    /\ deltasEmpty
...
\* Last modified Sun Dec 16 11:59:38 CET 2018 by tomek
