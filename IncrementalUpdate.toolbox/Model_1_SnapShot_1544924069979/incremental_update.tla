...
CONSTANTS LinkId, PotentialLinks
...
TypeOK ==
    /\ PotentialLinks \subseteq (LinkId \X LinkId)
...
ApplyDeltas ==
    ...
    /\ dLink' = [x \in PotentialLinks |-> 0]
    /\ UNCHANGED <<link>>

dLinkIsEmpty == BagEq(dLink, [x \in PotentialLinks |-> 0])

Init ==
    /\ link = [x \in PotentialLinks |-> 0]
    /\ dLink = [x \in PotentialLinks |-> 0]
    /\ dLinkIsEmpty
    /\ mHop = Hop(link, link)
...
Consistent ==
    ...
    /\ NoNegativeValues(link)
    /\ NoNegativeValues(mHop)

MaxDelta(n) ==
    n > SumF([i \in DOMAIN dLink |-> Abs(dLink[i])])

MaxLinkDups(n) ==
    \A t \in DOMAIN link : link[t] < n
...
\* Last modified Sun Dec 16 01:40:02 CET 2018 by tomek
